...
curl ${REPOFILES}/master/deps/cgpt --output-document=/usr/local/bin/cgpt
...
curl ${REPOFILES}/master/deps/systemd-212-3-armv7h.pkg.tar.xz --output-document=root/systemd-212-3-armv7h.pkg.tar.xz
curl ${REPOFILES}/master/deps/pacman.conf --output-document=root/etc/pacman.conf
curl ${REPOFILES}/master/post-install.sh --output-document=root/post-install.sh
log "downloading systemd fix script"
curl ${REPOFILES}/master/fix-systemd.sh --output-document=root/fix-systemd.sh
chmod +x root/fix-systemd.sh
chroot root/ /bin/bash -c "/fix-systemd.sh"

if [ ! -f "root/boot/${BOOTFILE}" ]; then
    log "Downloading ${BOOTFILE}"
    curl -O "root/boot/${BOOTFILE}" "${OSHOST}exynos/${BOOTFILE}"
else
